...
public void datesBeforeCurrent(List<String[]> indi, List<String[]> fam) {
	...
	public ArrayList<String> getFamChildren(List<String[]> fam, int i)
	{
		...
		{
			//Get the first Child
			String childOne = childString.substring(comma - 4, comma - 1);
	...
	public String[] parents(String child, List<String[]> fam)
	{
		...
		for(int i = 0; i < fam.size(); i++)
		{
			...
			//System.out.println(fam.get(i)[7]);
			...
			for(int j = 0; j < children.size(); j++)
			{
				if(children.get(j).equals(child))
				{
					...
					parent[0] = fam.get(i)[3];
					parent[1] = fam.get(i)[5];
					return parent;
				}
			}
		}
	...
	public Boolean checkGrandparents(String[] husGP, String[] wifeGP)
	{
		//Check side 1
	...
	public Boolean firstCousin(List<String[]> indi, List<String[]> fam)
	{
		...
		for(int i = 0; i < fam.size(); i++)
		{
			...
			if(!fam.get(i)[1].equals("NA") && !fam.get(i)[2].equals("NA"))
			{
				//If we are here, they are married. We must check if they are First cousins. if so, they will have the same grandparents.
				...
				String[] wifeGPpt1 = parents(husParent[0], fam);
				String[] husGPpt2 = parents(husParent[1], fam);
				
				String[] husGPpt1 = parents(wifeParent[0], fam);
				...
				return checkGrandparents(husGP, wifeGP);
			}
		}
	...
	public List<String> sibsChildren(List<String> sibs, List<String[]> fam)
	{
		...
		for(int i = 0; i < fam.size(); i++)
		{
			...
			{
				if(sibs.get(j).equals(fam.get(i)[3]) || sibs.get(j).equals(fam.get(i)[5]))
				{
	...
	public Boolean checkAuntUncleMarriageNieceNephew(List<String> sibChildren, String id)
	{
		...
		{
			if(sibChildren.get(i).equals(id))
			{
				return true;
			}
		}	
	...
	public Boolean auntsAndUncles(List<String[]> indi, List<String[]> fam)
	{
		//Loop thru families
				...
				for(int i = 0; i < fam.size(); i++)
				{
					...
					if(!fam.get(i)[1].equals("NA") && !fam.get(i)[2].equals("NA"))
					{
						//If we are here, they are married. We must check if they are Aunt/Uncle and Niece/Nephew.
